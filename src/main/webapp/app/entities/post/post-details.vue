    ...
    <div class="col-8">
      <div v-if="post">
        <h2 class="jh-entity-heading" data-cy="postDetailsHeading">
          <span v-text="$t('myApp.post.detail.title')">Post</span> {{ post.id }}
        </h2>
        <dl class="row jh-entity-details">
          <dt>
            <span v-text="$t('myApp.post.title')">Title</span>
          </dt>
          <dd>
            <span>{{ post.title }}</span>
          </dd>
          <dt>
            <span v-text="$t('myApp.post.content')">Content</span>
          </dt>
          <dd>
            <span>{{ post.content }}</span>
          </dd>
          <dt>
            <span v-text="$t('myApp.post.date')">Date</span>
          </dt>
          <dd>
            <span v-if="post.date">{{ $d(Date.parse(post.date), 'long') }}</span>
          </dd>
          <dt>
            <span v-text="$t('myApp.post.blog')">Blog</span>
          </dt>
          <dd>
            <div v-if="post.blog">
          ...
          <dt>
            <span v-text="$t('myApp.post.tag')">Tag</span>
          </dt>
          <dd>
